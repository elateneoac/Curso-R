...
También se pueden ver la cantidad de categorías de una variable con `lenght()` o `distinct()`
...
```
Algo muy importante es que el nombre de las variables de cada df, por las que queremos joinear, debe coincidir:
...
La función `left_join()` nos junta ambos df pero dándole prioridad a los valores del df de la izquierda, es decir, se queda con todos los valores del primer df y para los valores que no matchean con el segundo df, les asigna valores NA. Lo que obtenemos de esta manera es que no se modifiquen el número de filas o registros de nuestro primer df:
