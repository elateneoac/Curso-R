...
Ante un conjunto de datos, podemos querer conocer la estructura de las variables numéricas: 

```{r}
summary(df_divorcios)
```
Vemos que las variables no se ajustan al tipo de dato, por lo que lo modificaremos. Para ello, podemos utilizar otra función de la familia Applys, `lapply()`. 
...
#aplicamos la funcion as.numeric al conjunto de datos:
...
Podemos notar que -aunque estan próximas- la **mediana** < **media**, lo que evidencia un sesgo hacia la derecha (valores máximos).Esto puede quedar más claro con un **boxplot** mediante la función `boxplot()` o un histograma con `hist()`, ambos de R base: 
...
Por último, podríamos querer modificar la organización de los datos para realizar un join, visualizar o exportarlas a otro formato. 
...
#faltaría agregar los paquetes de resumenes estadísticos como "yapa", sumar lo de pivot wider/longer  y quizás plot no? que vean base esta clase y la 4 todo viz copada
