...
La clase pasada estuvimos introduciendonos en Tidyverse con algunas funciones básicas. Hoy vamos a profundizar en más funciones de limpieza y transformación de datos. 
...
#12 casos que registran la mayor cantidad de años de casadxs
```
...
```
La función  `plot()` de R permite realizar otros gráficos. 

```{r}

plot(factor(df_sin_outliers$MES_MAT), 
     main="Cantidad de divorcios por mes")

```

La función `pie()` permite dibujar un gráfico de torta (R es poco amigable con este tipo de gráficos)

```{r}
pie(divorcio_anual$total, 
    labels = divorcio_anual$ANIO_CREACION, 
    main = "Distribución de divorcios por año")
```
